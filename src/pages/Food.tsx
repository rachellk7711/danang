...
import { ShoppingBag, Utensils, Calendar, ChevronRight, Info, Tag, Coffee, Beer, Gift } from 'lucide-react';
...
const VIETNAMESE_FOOD_DATA = [
  ...
  { id: "vn_003", nameKr: "반쎄오", name: "Bánh Xèo", location: "다낭 / 호이안", ingredients: "돼지고기, 새우, 숙주", form: "쌀가루 크레이프 (부침개)", taste: "기름에 튀기듯 구워 고소하고 바삭함", tip: "라이스페이퍼에 바삭한 반쎄오와 허브를 듬뿍 넣고 돌돌 말아 소스에 찍어 드세요." },
  ...
  { id: "vn_010", nameKr: "카오러우", name: "Cao Lầu", location: "호이안 전용", ingredients: "차슈 (돼지고기)", form: "쌀국수 (굵고 쫄깃한 호이안 특산 면)", taste: "간장 및 오향 베이스의 짭짤한 맛", tip: "호이안에서만 맛볼 수 있는 쫄깃한 면발과 달콤짭짤한 고기 고명의 조화가 좋습니다." },
    ...
    <div className="pb-16 px-4 space-y-5">
      ...
      <section className="bg-navy-sub/20 rounded-2xl p-1 border border-white/5">
        ...
        {isDeliveryOpen && (
          <div className="px-4 pb-5 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
            <div className="space-y-3">
              <h4 className="text-xs font-bold text-text-hint ml-1 uppercase tracking-widest">다낭 지역 🏖️</h4>
              {DELIVERY_SPOTS_DANANG.map((spot, idx) => <DeliveryCard key={idx} spot={spot} />)}
            </div>
            <div className="space-y-3">
              <h4 className="text-xs font-bold text-text-hint ml-1 uppercase tracking-widest">호이안 지역 🏮</h4>
