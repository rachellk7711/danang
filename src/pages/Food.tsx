...
const VIETNAMESE_FOOD_DATA = [
  ...
  },
  {
    ...
    tip: "함께 나오는 바삭한 쌀과자를 부수어 면과 함께 비벼 먹는 것이 정석입니다."
  },
  {
    id: "vn_005",
    name: "Bánh Mì",
  ...
  },
  {
    ...
    tip: "버터 갈릭(Bơ Tỏi) 소스나 타마린드 소스 볶음을 추천합니다."
  },
  {
    id: "vn_008",
    name: "Nem Lụi",
  ...
  },
  {
    ...
    nameKr: "콤가",
    location: "호이안 추천",
    desc: "강황으로 노랗게 물들인 밥과 닭고기를 곁들인 베트남식 치킨라이스",
    tip: "호이안식 매운 고추 소스를 밥에 살짝 비벼 먹으면 더 맛있습니다."
  },
  {
    id: "vn_010",
    name: "Cao Lầu",
    nameKr: "카오러우",
    location: "호이안 전용",
    desc: "호이안에서만 맛볼 수 있는 쫄깃하고 굵은 면의 간장 베이스 비빔면",
    tip: "함께 나오는 바삭한 튀김 과자와 면의 식감을 즐겨보세요."
  },
  {
    id: "vn_011",
    ...
    nameKr: "화이트 로즈",
    location: "호이안 전용",
    desc: "투명하고 얇은 피 안에 새우를 넣은 장미 모양의 호이안 전통 만두",
    tip: "만두 위에 뿌려진 튀긴 마늘 후레이크와 함께 소스에 찍어 드세요."
  },
  {
    id: "vn_012",
    name: "Cháo",
  ...
  },
  {
    ...
    nameKr: "콤 빈전",
    location: "다낭/호이안",
    desc: "다양한 반찬 중 원하는 것을 골라 밥 위에 얹어 먹는 서민 식당",
    tip: "점심시간 로컬 시장 근처에서 가장 활기차고 맛있는 밥을 즐겨보세요."
  }
];
...
const FoodListItem: React.FC<{ food: any; index: number }> = ({ food, index }) => (
  <motion.div 
  ...
  >
    <div className="flex items-baseline justify-between mb-1.5">
      <div className="flex items-baseline gap-2">
        <span className="text-[14px] font-black text-mango">{food.nameKr}</span>
        <span className="text-[9px] text-text-hint font-medium uppercase opacity-60">{food.name}</span>
      </div>
      <span className="text-[9px] font-bold text-coral/90 px-1.5 py-0.5 bg-coral/10 rounded-md">
        {food.location}
      </span>
    </div>
    ...
    <div className="flex gap-2 items-start bg-white/3 rounded-lg p-2 border border-white/5">
      <div className="bg-mango/20 px-1.5 py-0.5 rounded text-[8px] font-black text-mango mt-0.5">TIP</div>
      <p className="text-[10px] text-text-primary leading-tight italic">
        {food.tip}
      </p>
    </div>
  </motion.div>
      ...
      <section className="bg-navy-sub/20 rounded-2xl p-0.5 border border-white/5">
        ...
        {isVnFoodOpen && (
          <div className="px-4 pb-4 animate-in fade-in slide-in-from-top-2 duration-300 divide-y divide-white/5">
            {VIETNAMESE_FOOD_DATA.map((food, index) => (
      ...
      <section className="bg-navy-sub/20 rounded-2xl p-0.5 border border-white/5">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-black text-mango tracking-tight">리조트 배달 맛집</h2>
          </div>
